...
//
    try{
        ...
        std::shared_ptr<Cont> c_c = std::make_shared<contcurent>(300, "lei", pf1, 5, 30, 20 );
    ...
    try{
        if(dynamic_cast<pers_fizica&>(*pf2).verificare_varsta()== 0) throw (eroare_varsta{"Minor, are nevoie de un tutore! \n"});
           ...
           // std::cout<<"-------------------------------pers_f 2----------------------"<<std::endl;
                std::shared_ptr<Cont> c_s = std::make_shared<cont_standard>(10000, "euro",  pf2,5,  10, 0.05);
    ...
    try {
        std::shared_ptr<Cont> c_p = std::make_shared<cont_premium>( 700, "euro",pj, 5, 50    );
